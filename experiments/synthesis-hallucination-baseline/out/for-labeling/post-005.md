...
> Five days of commits on a social engagement pipeline, and the shape that emerges is: stop hardcoding assumptions, start measuring everything.
> 
> It started with extraction hardening — config-driven Puppeteer fallback hosts for sites like Discord and Stripe that don't render without a real browser. A list in YAML instead of a constant buried in source. Small, but it signals the right instinct: operators should control which domains get expensive treatment.
> 
> Then the voice system got rebuilt from the ground up. Haiku extraction — pulling voice patterns directly from a model — got replaced with statistical voice dice and a 4-stage exposure pool. Move probabilities now come from measured data on published posts, not inference. That's a meaningful architectural shift: the system's personality now has a documented prior, not a live one.
> 
> Ranking followed. A `computeCandidateRankingScore` function with explicit fire count penalties, a `max_daily_posts_per_author` config (default 2, enforced as a min-1 integer), and a `getDraftsSince` storage method so the ranker knows what's already been queued. The `openingMove` field got added to the post type. Every one of those is a constraint that previously lived in someone's head.
> 
> Article matching got an enrichment layer: findings now carry `retrievalText` and `retrievalTerms`, the stage1 matcher builds semantic queries with fallback thresholds, and security/integration modules emit richer domain signals including KMS and envelope encryption details. Better signal in, better match out.
> 
> Last: the promotion limit that was hardcoded at 20 became `CONTENT_FETCH_PROMOTION_LIMIT`, settable via workflow dispatch or env var. One operator input that unlocks runtime control without a deploy.
> 
> Five commits. Every one removes a hardcoded assumption.
> 
...
### Commit 2: 9b63ce4
**Message:** feat: make content fetch promotion limit configurable

**Diff:**
```diff
--- .github/workflows/content-fetch.yml
diff --git a/.github/workflows/content-fetch.yml b/.github/workflows/content-fetch.yml
index ca06328..1efefd9 100644
--- a/.github/workflows/content-fetch.yml
+++ b/.github/workflows/content-fetch.yml
@@ -5,6 +5,11 @@ on:
     # Sunday 6am CLT = 10:00 UTC (CLT = UTC-4 in summer)
     - cron: '0 10 * * 0'
   workflow_dispatch:
+    inputs:
+      promotion_limit:
+        description: 'Queued sources to promote this run'
+        type: number
+        default: 20
...
+          CONTENT_FETCH_PROMOTION_LIMIT: ${{ inputs.promotion_limit || vars.CONTENT_FETCH_PROMOTION_LIMIT || '20' }}
...
@@ -59,4 +59,5 @@ jobs:
           ...
           ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
           OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
+          CONTENT_FETCH_PROMOTION_LIMIT: ${{ vars.CONTENT_FETCH_PROMOTION_LIMIT || '20' }}
         run: npx tsx src/content/scripts/content-fetch-main.ts


--- docs/devcast-spec.md
diff --git a/docs/devcast-spec.md b/docs/devcast-spec.md
index 38c042b..6a614cf 100644
...
|---|-------|-----------------|----------|-------|
| 0 | Five days of commits on a social engagement pipeline, and the shape that emerges is: stop hardcoding assumptions, start measuring everything. | needs_human | | |
| 1 | It started with extraction hardening — config-driven Puppeteer fallback hosts for sites like Discord and Stripe that don't render without a real browser. | needs_human | | |
| 2 | A list in YAML instead of a constant buried in source. | needs_human | | |
| 3 | Small, but it signals the right instinct: operators should control which domains get expensive treatment. | needs_human | | |
| 4 | Then the voice system got rebuilt from the ground up. | needs_human | | |
| 5 | Haiku extraction — pulling voice patterns directly from a model — got replaced with statistical voice dice and a 4-stage exposure pool. | needs_human | | |
| 6 | Move probabilities now come from measured data on published posts, not inference. | needs_human | | |
| 7 | That's a meaningful architectural shift: the system's personality now has a documented prior, not a live one. | needs_human | | |
| 8 | A `computeCandidateRankingScore` function with explicit fire count penalties, a `max_daily_posts_per_author` config (default 2, enforced as a min-1 integer), and a `getDraftsSince` storage method so t | needs_human | | |
| 9 | The `openingMove` field got added to the post type. | needs_human | | |
| 10 | Every one of those is a constraint that previously lived in someone's head. | needs_human | | |
| 11 | Article matching got an enrichment layer: findings now carry `retrievalText` and `retrievalTerms`, the stage1 matcher builds semantic queries with fallback thresholds, and security/integration modules | needs_human | | |
| 12 | Better signal in, better match out. | needs_human | | |
| 13 | Last: the promotion limit that was hardcoded at 20 became `CONTENT_FETCH_PROMOTION_LIMIT`, settable via workflow dispatch or env var. | needs_human | | |
| 14 | One operator input that unlocks runtime control without a deploy. | needs_human | | |
| 15 | Every one removes a hardcoded assumption. | needs_human | | |

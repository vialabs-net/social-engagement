...
> Twelve days, six commits, one CRM shipped from zero.
> 
> Day 1: database models for Transaction, Client, Vehicle, Document, and Delivery land alongside the route layer and a web UI. Not a prototype—a foundation with real structure.
> 
> Day 5: the first behavioral seam appears. A static transaction detail page gets replaced by TransactionDetailLive, a client component that polls for status changes and fires a toast when a transaction hits READY. The UI now reacts to the backend instead of waiting for a refresh.
> 
> Day 6 (morning): delivery gets a proper contract. Four status values—PENDING_CLIENT_MATCH, PENDING_IDAUTO_DELIVERY, DELIVERED_TO_APP, DELIVERY_FAILED—replace whatever "it either worked or it didn't" logic existed before. A notification result shape (kind, externalDeliveryId, message, error) makes failure observable, not silent. The endpoint and auth are behind env vars with a TODO to swap in the real backend, which is honest engineering.
> 
> Day 6 (afternoon): notes and follow-ups. New models, new enums, new endpoints, new UI. Demo data seeded against a known transaction ID so the feature has something to render before the database catches up.
> 
> Day 12: CI pipeline, Docker images for both the web app and the BFF, a Helm chart, and GCP Artifact Registry integration. The thing can now actually deploy.
> 
> Same day, late: a layout system crystallizes. DashboardPageFrame, navbar, sidebar—centralized structure instead of per-page assembly. Session handling gets error paths and login redirects, which means auth failures now go somewhere intentional.
> 
> Twelve days from models to Kubernetes. The scope was real and the sequencing was deliberate.
> 
...
|---|-------|-----------------|----------|-------|
| 0 | Twelve days, six commits, one CRM shipped from zero. | needs_human | | |
| 1 | Day 1: database models for Transaction, Client, Vehicle, Document, and Delivery land alongside the route layer and a web UI. | needs_human | | |
| 2 | Not a prototype—a foundation with real structure. | needs_human | | |
| 3 | Day 5: the first behavioral seam appears. | needs_human | | |
| 4 | A static transaction detail page gets replaced by TransactionDetailLive, a client component that polls for status changes and fires a toast when a transaction hits READY. | needs_human | | |
| 5 | The UI now reacts to the backend instead of waiting for a refresh. | needs_human | | |
| 6 | Day 6 (morning): delivery gets a proper contract. | needs_human | | |
| 7 | Four status values—PENDING_CLIENT_MATCH, PENDING_IDAUTO_DELIVERY, DELIVERED_TO_APP, DELIVERY_FAILED—replace whatever "it either worked or it didn't" logic existed before. | needs_human | | |
| 8 | A notification result shape (kind, externalDeliveryId, message, error) makes failure observable, not silent. | needs_human | | |
| 9 | The endpoint and auth are behind env vars with a TODO to swap in the real backend, which is honest engineering. | needs_human | | |
| 10 | Day 6 (afternoon): notes and follow-ups. | needs_human | | |
| 11 | New models, new enums, new endpoints, new UI. | needs_human | | |
| 12 | Demo data seeded against a known transaction ID so the feature has something to render before the database catches up. | needs_human | | |
| 13 | Day 12: CI pipeline, Docker images for both the web app and the BFF, a Helm chart, and GCP Artifact Registry integration. | needs_human | | |
| 14 | The thing can now actually deploy. | needs_human | | |
| 15 | Same day, late: a layout system crystallizes. | needs_human | | |
| 16 | DashboardPageFrame, navbar, sidebar—centralized structure instead of per-page assembly. | needs_human | | |
| 17 | Session handling gets error paths and login redirects, which means auth failures now go somewhere intentional. | needs_human | | |
| 18 | Twelve days from models to Kubernetes. | needs_human | | |
| 19 | The scope was real and the sequencing was deliberate. | needs_human | | |

...
> Five days, three commits, one CRM going from schema to something a real user can actually wait on.
> 
> The foundation commit landed all at once — Client, Vehicle, Transaction, Document, WorkflowEvent, Delivery models, plus transaction routes wired into the app and email/password auth with reset flows. That's the kind of commit that represents weeks of design work compressed into a single merge: every entity relationship had to be decided before a line of schema could be written.
> 
> Then the interesting problem showed up: transactions don't complete instantly. Something downstream processes them and eventually flips the status to READY. The original page had no way to surface that transition live. The fix was to extract a `TransactionDetailLive` client component that polls and fires a toast notification the moment that status change lands. The key architectural move here is the boundary — keeping the polling isolated in a client component instead of tangling it into server-rendered page logic. When the status is READY, the user knows without refreshing.
> 
> The third commit adds date-range filtering alongside the existing status filter, and the detail is in the discipline: `from` and `to` flow all the way through BFF route, query schema, service layer, and frontend with date normalization at the edges. A filter that exists in the UI but gets dropped somewhere in the stack is just noise. Threading it end-to-end with explicit schema validation means the constraint is enforced, not hoped for.
> 
> A working CRM in five days isn't magic — it's deciding the data model early and not leaving sharp edges in the operational layer.
> 
...
|---|-------|-----------------|----------|-------|
| 0 | Five days, three commits, one CRM going from schema to something a real user can actually wait on. | needs_human | | |
| 1 | The foundation commit landed all at once — Client, Vehicle, Transaction, Document, WorkflowEvent, Delivery models, plus transaction routes wired into the app and email/password auth with reset flows. | needs_human | | |
| 2 | That's the kind of commit that represents weeks of design work compressed into a single merge: every entity relationship had to be decided before a line of schema could be written. | needs_human | | |
| 3 | Then the interesting problem showed up: transactions don't complete instantly. | needs_human | | |
| 4 | Something downstream processes them and eventually flips the status to READY. | needs_human | | |
| 5 | The original page had no way to surface that transition live. | needs_human | | |
| 6 | The fix was to extract a `TransactionDetailLive` client component that polls and fires a toast notification the moment that status change lands. | needs_human | | |
| 7 | The key architectural move here is the boundary — keeping the polling isolated in a client component instead of tangling it into server-rendered page logic. | needs_human | | |
| 8 | When the status is READY, the user knows without refreshing. | needs_human | | |
| 9 | The third commit adds date-range filtering alongside the existing status filter, and the detail is in the discipline: `from` and `to` flow all the way through BFF route, query schema, service layer, a | needs_human | | |
| 10 | A filter that exists in the UI but gets dropped somewhere in the stack is just noise. | needs_human | | |
| 11 | Threading it end-to-end with explicit schema validation means the constraint is enforced, not hoped for. | needs_human | | |
| 12 | A working CRM in five days isn't magic — it's deciding the data model early and not leaving sharp edges in the operational layer. | needs_human | | |
